...
class RGBConverter(context: Context){
    ...
    fun yuvToRgb(image: Image, output: Bitmap){
        ...
        if (!::inputAllocation.isInitialized) inputAllocation = Allocation.createSized(rs, Element.U8(rs), yuvBuffer.array().size)
        if (!::outputAllocation.isInitialized) outputAllocation = Allocation.createFromBitmap(rs, output)
    ...
    private fun imageToByteBuffer(image: Image, outputBuffer: ByteBuffer){
        ...
        imagePlanes.forEachIndexed{ planeIndex, plane ->
            ...
            else Rect(imageCrop.left / 2, imageCrop.top / 2, imageCrop.right / 2, imageCrop.bottom / 2)
